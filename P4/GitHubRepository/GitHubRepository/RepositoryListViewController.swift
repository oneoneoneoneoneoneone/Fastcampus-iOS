    ...
    func fetchRepositories(of organization: String){
        ...
        Observable.from([organization])
            ...
            .flatMap{ request -> Observable<(response: HTTPURLResponse, data: Data)> in
                //rx - 파운데이션(ex URLSession), UIKit 기본인자를 rx로 번환 가능
                //response() 반환값 - Observable<(response: HTTPURLResponse, data: Data)
                return URLSession.shared.rx.response(request: request)
            }
            //response 조건 필터링, data X
            .filter{ response, _ in
                return 200..<300 ~= response.statusCode
            }
            //response X,data array 형태로 변환
            ...
            .map{ objects in
                //compactMap - 디코딩 실패시 배출된 nil 값을 자동 제거
